function genMouseEvent() {
	...
	var filecopyPath = fl.configURI + 'WindowSWF/' + "drcascripts/code_gen_mouseevent_as3.as";
...
function genReflection() {
	...
	if (isLayerExisted) {		
		...
		var filecopyPath = fl.configURI + 'WindowSWF/' + "drcascripts/ReflectionAS3.as";
		var connectionscripts = FLfile.read(filecopyPath);
		if (connectionscripts) {
	...
	var filecopyPath = fl.configURI + 'WindowSWF/' + "drcascripts/code_gen_reflection_as3.as";
	var connectionscripts = FLfile.read(filecopyPath);
	if (connectionscripts) {
		var actionScript = connectionscripts;
		actionScript = actionScript.split("instance").join(instanceName);

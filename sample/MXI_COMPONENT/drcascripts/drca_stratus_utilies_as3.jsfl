function initStratusConnection() {
	...
	if (isLayerExisted) {
		...
		var filecopyPath = fl.configURI + 'WindowSWF/' + "drcascripts/red_connection_as3.as";
	...
	if( funcName == null ) {
		...
		if (isLayerExisted) {	
			...
			if (funcName != null) {
				...
				layer.frames[0].actionScript += "\n" + "function " + funcName + "(params: Object) {";
				if (funcStatement != null) {
					layer.frames[0].actionScript += "\n\t" + funcStatement;
				}
				layer.frames[0].actionScript += "\n" + "}";
				prompt("Copy this text and insert into your script", 'GlobalVars.getInstance().sendAction("' + funcName + '");');
			}
		} else {

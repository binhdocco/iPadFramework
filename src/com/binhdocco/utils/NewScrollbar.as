...
package com.binhdocco.utils{
	...
	public class NewScrollbar {
		...
		private var thescrollbar: MovieClip;
		

		public function NewScrollbar(mc: MovieClip):void {
			thescrollbar = mc;
			scroller = mc.scroller;
			track = mc.track;
		...
		public function init(t:MovieClip, tr:String,tt:Number,sa:Boolean,b:Number):void {
			...
			sBuffer = b;		
			scroller.y = 0;
			thescrollbar.visible = true;
			if (target.height <= track.height) {
				thescrollbar.visible = false;
			}			
		...
		public function dragScroll(event:MouseEvent):void {			
			...
			thescrollbar.stage.addEventListener(MouseEvent.MOUSE_MOVE, moveScroll);
			thescrollbar.stage.addEventListener(MouseEvent.MOUSE_UP, stopScroll);
		}
		//
		...
		public function stopScroll(event:MouseEvent):void {
			...
			isDown = false;
			scroller.stopDrag();
			thescrollbar.stage.removeEventListener(MouseEvent.MOUSE_MOVE, moveScroll);
			thescrollbar.stage.removeEventListener(MouseEvent.MOUSE_UP, stopScroll);
		}
		//
		...
		public function startScroll():void {
			...
			target.y = -sPos;
			//TweenNano.to(target, timing, {y:-sPos, ease:trans});
		}
	}
}